...
export default function sidebar() {
    ...
    async function offlineStatus(docRef) {
...
async function onlineStatus(docRef) {
    if (docRef) {
        await updateDoc(docRef, {
            onlineStatus: true,
            lastSeen: serverTimestamp()
...
async function monitorLoggedInFriends(userId) {
    // Limpa listeners anteriores para evitar múltiplos registros
    ...
    try {
        ...
        const unsubscribe = onSnapshot(friendsQuery, (querySnapshot) => {
            // console.clear(); // Limpa o console para evitar spam de logs repetidos
            querySnapshot.forEach((friendDoc) => {
                if (friendDoc.exists()) {
                    const friendData = friendDoc.data();

                    if (friendData.onlineStatus) {
                        console.log(`Amigo online: ${friendData.username} (ID: ${friendDoc.id})`);
                    } else {
                        console.log(`Amigo offline: ${friendData.username} (ID: ${friendDoc.id})`);
                    }
                }
            });

            console.log('Monitoramento em tempo real atualizado.');
        });

        unsubscribeFunctions.push(unsubscribe);

    } catch (error) {
        console.error("Erro ao monitorar amigos: ", error);
    }
}
